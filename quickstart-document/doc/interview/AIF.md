...
3、Activemq集群模型的优化，broker故障隔离
4、连接池的使用：对象、连接统一管控，动态调整（线程池），消息对象使用对象池的技术
5、主题队列的负载均衡：主题的分片，发送负载路由（提升并发TPS）【activemq对broker的负载、broker故障隔离】
6、消费端去重（Redis的setnx命令，数据库）
7、消息轨迹跟踪（ES,Jest(searchbox)作为客户端查询）
8、消息的稽核（按天分库分表，先使用yugong同步数据，然后再定时任务稽核（根据参数分中心，分主题等稽核），或者使用ET框架执行任务）
9、消息序列化提供：(Kryo和FST)
10、消息压缩
11、消息加解密的发送
12、消息的监控告警
13、权限的控制（发送和消费的权限控制、黑白名单等）
14、消费端消费的mq积压之后，熔断退出由其他消费者消费
15、消息分析（业务端开发）
16、消息异常（生产、消费）的持久化
