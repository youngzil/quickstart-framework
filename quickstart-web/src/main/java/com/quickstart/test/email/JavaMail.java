 ...
 * 
 * 准备一个properties文件，该文件中存放SMTP服务器地址等参数。 利用properties创建一个Session对象 利用Session创建Message对象，然后设置邮件主题和正文 利用Transport对象发送邮件
 * 
...
public class JavaMail {
    ...
    /***
     * 邮件是既可以被发送也可以被受到。JavaMail使用了两个不同的类来完成这两个功能：Transport 和 Store。 Transport 是用来发送信息的，而Store用来收信。对于这的教程我们只需要用到Transport对象。
     */
