...
#  npm i -g aid-cli@2.0.0 --sass-binary-site=http://npm.taobao.org/mirrors/node-sass/
  ...
  echo "[INFO] NVM版本:" `nvm --version`
  echo "[INFO] NodeJS版本:" `node -v`
  echo "[INFO] npm版本:" `npm -v`
  echo "[INFO] aid版本:" `aid -v`
}
...
setProjectVersion(){
  ...
  then
      echo "[ERROR] 项目${PROJECT_NAME}的POM配置文件有误，请检查"
      echo "[ERROR] $PROJECT_VERSION"
  else
      echo "[INFO] 当前正在打包[分支=${BRANCH_NAME},工程=${PROJECT_NAME},版本=${PROJECT_VERSION}]"
  fi
}
...
aifgw()
{
  ...
  echo "[INFO] 开始编译${PROJECT_NAME}..."

  if [ -d $CODE_PATH/local/$PROJECT_NAME ]; then
    echo "[INFO] 开始替换代码${PROJECT_NAME}..."
  ...
  if [ ! -f $CODE_PATH/$PROJECT_NAME/aifgw-distribution/target/$TARGET_NAME ];then
          echo "[ERROR] 打包失败,请确认${PROJECT_NAME}打包是否成功"
          exit 1
  fi
  ...
  echo "[INFO] 项目${PROJECT_NAME}编译完成,打包文件的文件请查看$TAR_PACKAGE_PATH/$TARGET_NAME..."
}
...
#  echo "[INFO] aid版本:" `aid -v`
  ...
  aid build -N
  if [ -d "dist" ];then
  echo "[INFO] 开始打包${PROJECT_NAME}的war包..."
      cd dist
      jar cf $TARGET_NAME *
      rm -rf $TAR_PACKAGE_PATH/$TARGET_NAME
      mv $TARGET_NAME $TAR_PACKAGE_PATH/
  fi
  echo "[INFO] ${TARGET_NAME}打包完成,打包文件的文件请查看$TAR_PACKAGE_PATH/$TARGET_NAME..."
}
...
#  echo "[INFO] aid版本:" `aid -v`
  ...
  aid build -N
  if [ -d "dist" ];then
  echo "[INFO] 开始打包$TARGET_NAME的war包..."
      cd dist
      jar cf $TARGET_NAME *
      rm -rf $TAR_PACKAGE_PATH/$TARGET_NAME
      mv $TARGET_NAME $TAR_PACKAGE_PATH/
...
echo "[INFO] #####aifgw 版本:${ospversion}#####"
echo "[INFO] 打包完成,耗时=${STAMP_DIFF}秒，打包后的压缩包请查看$TAR_PACKAGE_PATH..."
