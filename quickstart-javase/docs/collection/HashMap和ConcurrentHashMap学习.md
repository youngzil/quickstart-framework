...
- [JDK1.8中HashMap底层链表转红黑树的阈值为什么是8？红黑树转链表为什么是6？](#JDK1.8中HashMap底层链表转红黑树的阈值为什么是8但是红黑树转链表为什么是6)
...
[HashMap 和 HashTable 以及 ConcurrentHashMap的原理和区别](https://blog.csdn.net/songzi1228/article/details/99974540)
...
1、当数组长度为2的幂次方时，可以使用位运算来计算元素在数组中的下标，减少resize扩容过程中元素移动的数量  
   可以通过位运算e.hash & (newCap - 1)来计算，a % (2^n) 等价于 a & (2^n - 1)  ，位运算的运算效率高于算术运算，原因是算术运算还是会被转化为位运算
2、 增加hash值的随机性，减少hash冲突,让哈希后的结果更加均匀


[看下JDK8中的hash算法](怎么设计hashcode更均衡.md#看下JDK8中的hash算法)  
...
## JDK1.8中HashMap底层链表转红黑树的阈值为什么是8但是红黑树转链表为什么是6
