...
public class StreamApiTest {
    ...
    public Map<String, List<Article>> groupByAuthor2() {
        ...
        Map<String, List<Article>> map = articles.stream().filter(article -> article.getTags().contains("Javafffff"))
    ...
    public void testTransformStream() throws InterruptedException {
        ...
        System.out.println(
            "sum is:" + ss.sum());// java.lang.IllegalStateException: stream has already been operated upon or closed
    ...
    public void test1() {
        ...
        List<Transaction> filterList =
            transactions.stream().filter(t -> t.getMoney() > 20).collect(Collectors.toList());
        // 满足条件的循环处理
    ...
    @Test
    public void test2() {
        Map<Currency, List<Transaction>> currencyListMap =
            transactions.stream().collect(Collectors.groupingBy(Transaction::getCurrency));
        Map<Currency, List<Transaction>> currencyListMap1 =
            transactions.stream().collect(Collectors.groupingBy(t -> t.getCurrency()));
