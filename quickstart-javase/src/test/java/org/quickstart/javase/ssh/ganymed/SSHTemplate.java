...
public class SSHTemplate {
    ...
    private static ThreadPoolExecutor taskPool =
            new ThreadPoolExecutor(200, 200, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(1000), new ThreadFactoryBuilder().setNameFormat("SSH-%d").setDaemon(true).build());

    public Result execute(String ip, SSHCallback callback) throws Exception {
        return execute(ip, ConstUtils.DEFAULT_SSH_PORT_DEFAULT, ConstUtils.USERNAME, ConstUtils.PASSWORD, callback);
    }
     ...
     * @throws SSHException
     */
    public Result execute(String ip, int port, String username, String password, SSHCallback callback) throws Exception {
        Connection conn = null;
        try {
            conn = getConnection(ip, port, username, password);
            return callback.call(new SSHSession(conn, ip + ":" + port));
        } catch (Exception e) {
            throw new Exception("SSH err: " + e.getMessage(), e);
        } finally {
            close(conn);
        }
    }
    ...
    public class SSHSession {
        ...
        public Result executeCommand(final Session session, final String cmd, final int timoutMillis, final LineProcessor lineProcessor) throws Exception {
        ...
        public Result scp(String[] localFiles, String[] remoteFiles, String remoteTargetDirectory, String mode) {
            try {
                ...
                logger.error("scp local=" + Arrays.toString(localFiles) + " to " + remoteTargetDirectory + " remote=" + Arrays.toString(remoteFiles) + " err", e);
                return new Result(e);
            }
        }
