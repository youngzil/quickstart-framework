package org.quickstart.javase.jmx.examples.DynamicMBean;
/*
 ...
 *
 * The "SimpleDynamic" dynamic MBean shows how to expose for management attributes and operations, at runtime, by implementing the "javax.management.DynamicMBean" interface.
 *
 * This MBean exposes for management two attributes and one operation: - the read/write "State" attribute, - the read only "NbChanges" attribute, - the "reset()" operation. It does so by putting this
 * information in an MBeanInfo object that is returned by the getMBeanInfo() method of the DynamicMBean interface.
 *
 * It implements the access to its attributes through the getAttribute(), getAttributes(), setAttribute(), and setAttributes() methods of the DynamicMBean interface.
 *
 * It implements the invocation of its reset() operation through the invoke() method of the DynamicMBean interface.
 * 
 * Note that as "SimpleDynamic" explicitly defines one constructor, this constructor must be public and exposed for management through the MBeanInfo object.
 */
...
public class SimpleDynamic implements DynamicMBean {
    ...
    /**
     * Return the "NbResets" property. This method is not a Getter in the JMX sense because it is not returned by the getMBeanInfo() method.
     */
    ...
    /**
     * Build the private dMBeanInfo field, which represents the management interface exposed by the MBean; that is, the set of attributes, constructors, operations and notifications which are
     * available for management.
     *
     * A reference to the dMBeanInfo object is returned by the getMBeanInfo() method of the DynamicMBean interface. Note that, once constructed, an MBeanInfo object is immutable.
     */
